...
tropical_glaciers |> st_drop_geometry() |> group_by(ecosystem_name) |> summarise(total = n(), .groups = "drop")


glaciers_on_volcanos <- tropical_glaciers |> filter(ecosystem_name %in% "Volcanos de Peru y Chile")
table(glaciers_on_volcanos$crs_code)
glaciers_on_volcanos <- st_transform(glaciers_on_volcanos, crs = 32719)
...
```{r Getting number of AOO grids}
glaciers_VPC_B2_with_evidence <- thresholds(AOO_grid, 
